...
public class MoonUtil {
	...
	private static final String SP_NAME = "InputPanel";
	...
	public static void identifyRecentVHFaceExpressionAndTags(Context context, View textView,
		...
		SpannableString mSpannableString = makeSpannableStringTags(null,context, value, scale, align, false);
		TeamMemberAitHelper.replaceAitForeground(value, mSpannableString);
		viewSetText(textView, mSpannableString);
	}

	public static void identifyRecentVHFaceExpressionAndTags(RecentContact recentContact, Context context, View textView,
                                                             String value, int align, float scale) {
		SpannableString mSpannableString = makeSpannableStringTags(recentContact,context, value, scale, align, false);
		TeamMemberAitHelper.replaceAitForeground(value, mSpannableString);
		if (value.indexOf("[草稿]") != -1){
	...
	public static SpannableString makeSpannableStringTags(Context context, String value, float scale, int align) {
		return makeSpannableStringTags(null,context, value, DEF_SCALE, align, true);
	}

	public static SpannableString makeSpannableStringTags(RecentContact recentContact, Context context, String value, float scale, int align, boolean bTagClickable) {
		...
		if(value.indexOf("优惠券") != -1){

			ForegroundColorSpan colorSpan = new ForegroundColorSpan(Color.parseColor("#ee4b62"));
			mSpannableString.setSpan(colorSpan, value.indexOf("优惠券") , value.indexOf("优惠券")+3, Spannable.SPAN_INCLUSIVE_EXCLUSIVE);

		}

		if(value.indexOf("[草稿]") != -1){
			ForegroundColorSpan colorSpan = new ForegroundColorSpan(Color.parseColor("#ee4b62"));
			mSpannableString.setSpan(colorSpan, value.indexOf("[草稿]") , value.indexOf("[草稿]")+4, Spannable.SPAN_INCLUSIVE_EXCLUSIVE);

		}

		if (recentContact != null){
			//判断是否有草稿
			SharedPreferences sp = NimUIKit.getContext().getSharedPreferences(SP_NAME, Context.MODE_PRIVATE);;
			String contactid = recentContact.getContactId();
			String draft = sp.getString(contactid, "");
			if (!draft.equals("")){
				draft =  "[草稿]"+draft;
				mSpannableString = new SpannableString(draft);
				ForegroundColorSpan colorSpan = new ForegroundColorSpan(Color.parseColor("#ee4b62"));
				mSpannableString.setSpan(colorSpan, draft.indexOf("[草稿]") , draft.indexOf("[草稿]")+4, Spannable.SPAN_INCLUSIVE_EXCLUSIVE);
		...
		if(value.indexOf("伊点卡") !=-1){

			ForegroundColorSpan colorSpan = new ForegroundColorSpan(Color.parseColor("#ee4b62"));
			mSpannableString.setSpan(colorSpan, value.indexOf("伊点卡"), value.indexOf("伊点卡")+3, Spannable.SPAN_INCLUSIVE_EXCLUSIVE);
