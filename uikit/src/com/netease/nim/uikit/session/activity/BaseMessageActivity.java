...
public abstract class BaseMessageActivity extends UI {
    ...
    private void parseIntent() {
        ...
        boolean isSystem = getIntent().getBooleanExtra(Extras.SYSTEMMSG, false);
        if (!isSystem){
            if (customization != null) {
    ...
    private void addRightCustomViewOnActionBar(UI activity, List<SessionCustomization.OptionsButton> buttons) {
        ...
        mView = (LinearLayout) LayoutInflater.from(activity).inflate(R.layout.nim_action_bar_custom_view, null);
        ...
        for (final SessionCustomization.OptionsButton button : buttons) {
            ...
            mView.addView(imageView, params);
        }

        toolbar.addView(mView, new Toolbar.LayoutParams(ViewGroup.LayoutParams.WRAP_CONTENT, ViewGroup.LayoutParams.MATCH_PARENT, Gravity.RIGHT | Gravity.CENTER));
    }
