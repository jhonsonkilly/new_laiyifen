...
public class TeamNotificationHelper {
    ...
    private static String buildUpdateTeamNotification(String tid, String account, UpdateTeamAttachment a) {
        ...
        for (Map.Entry<TeamFieldEnum, Object> field : a.getUpdatedFields().entrySet()) {
            if (field.getKey() == TeamFieldEnum.Name) {
                ...
                sb.append("群欢迎语被更新为 " + field.getValue());
    ...
    private static String buildAcceptInviteNotification(String from, MemberChangeAttachment a, String tid) {
        StringBuilder sb = new StringBuilder();
        try {
            String con = "欢迎" + getTeamMemberDisplayName(from) + "加入群" + TeamDataCache.getInstance().getTeamById(tid).getName();
            //        String con = getTeamMemberDisplayName(from) + " 接受了 " + buildMemberListString(a.getTargets(), null) + " 的入群邀请";
            //        sb.append(getTeamMemberDisplayName(from));
            //        sb.append(" 接受了 ").append(buildMemberListString(a.getTargets(), null)).append(" 的入群邀请");

            if (TeamDataCache.getInstance().getTeamById(tid).getExtension() != null) {
                sb.append(TeamDataCache.getInstance().getTeamById(tid).getExtension().equals("") ? con : TeamDataCache.getInstance().getTeamById(tid).getExtension() + "(" + con + ")");
            } else {
                sb.append(con);
            }
        }catch (Exception e){
