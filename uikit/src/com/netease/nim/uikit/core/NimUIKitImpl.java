...
public final class NimUIKitImpl {
    ...
    */
    ...
    public static void startP2PSession(Context context, String account, boolean isSystemMsg) {
        startP2PSession(context, account, null, isSystemMsg);
    }

    public static void startP2PSession(Context context, String account, IMMessage anchor, boolean isSystemMsg) {
        NimUIKitImpl.startChatting(context, account, SessionTypeEnum.P2P, commonP2PSessionCustomization, anchor, isSystemMsg);
    }

    public static void startP2PSession2(Context context, String account, IMMessage anchor, boolean isSystemMsg) {
        NimUIKitImpl.startChatting2(context, account, SessionTypeEnum.P2P, commonP2PSessionCustomization, anchor, isSystemMsg);
    }
    ...
    public static void startTeamSession(Context context, String tid, IMMessage anchor) {
        NimUIKitImpl.startChatting(context, tid, SessionTypeEnum.Team, commonTeamSessionCustomization, anchor, false);
    }

    public static void startChatting(Context context, String id, SessionTypeEnum sessionType, SessionCustomization
            customization, IMMessage anchor, boolean isSystemMsg) {
        if (sessionType == SessionTypeEnum.P2P) {
            //在全局记录一下 聊天对象是否为好友
            boolean isFriend = false;
            for (NimUserInfo nimUserInfo : NimUserInfoCache.getInstance().getAllUsersOfMyFriend()) {
                if (nimUserInfo.getAccount().equals(id)) {
                    isFriend = true;
                }
            }
            NimUIKit.isFriend = isFriend;
            NimUIKit.chatId = id;

            P2PMessageActivity.start(context, id, customization, anchor, isSystemMsg,"陌生人聊天添加");
        } else if (sessionType == SessionTypeEnum.Team) {
            NimUIKit.isFriend = true;
            NimUIKit.chatId = id;
    ...
    public static void startChatting2(Context context, String id, SessionTypeEnum sessionType, SessionCustomization
        ...
        if (sessionType == SessionTypeEnum.P2P) {
            ...
            P2PMessageActivity.start2(context, id, customization, anchor, isSystemMsg,"陌生人聊天添加");
        } else if (sessionType == SessionTypeEnum.Team) {
            NimUIKit.isFriend = true;
            NimUIKit.chatId = id;
            TeamMessageActivity.start(context, id, customization, null, anchor);
    ...
    public static void startChatting(Context context, String id, SessionTypeEnum sessionType, SessionCustomization customization,
                                     Class<? extends Activity> backToClass, IMMessage anchor, boolean isSystemMsg) {
    ...
    public static void startContactSelector(Fragment fragment, Context context, ContactSelectActivity.Option option, int requestCode) {
        ContactSelectActivity.startActivityForResult(fragment, context, option, requestCode);
    }
