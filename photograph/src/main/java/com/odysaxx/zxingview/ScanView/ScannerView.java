package com.odysaxx.zxingview.ScanView;
...
import com.odysaxx.zxingview.CameraPreview;
import com.odysaxx.zxingview.CameraUtils;
import com.odysaxx.zxingview.DisplayUtils;
import com.odysaxx.zxingview.RGBLuminanceSourceLocal;
...
//            stopCamera();
     ...
     * 解析图像
     */
    public void scanDecode(Bitmap bmap) {
        if (bmap == null) {
            return;
        }
        RGBLuminanceSourceLocal source = new RGBLuminanceSourceLocal(bmap);
        //将图片转换成二进制图片
        Result rawResult = null;
        if (source != null) {
            BinaryBitmap bitmap = new BinaryBitmap(new HybridBinarizer(source));
