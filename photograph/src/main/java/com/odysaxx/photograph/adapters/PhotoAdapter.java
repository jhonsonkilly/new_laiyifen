    ...
    private void initMultiMode() {
        ...
        mOnPhotoClick = new View.OnClickListener() {
            ...
            public void onClick(View v) {
                String path = (String) v.findViewById(R.id.checkmark).getTag();
    ...
    public View getView(int position, View convertView, ViewGroup parent) {
        if(getItemViewType(position) == TYPE_CAMERA) {
            ...
            if(mSelectMode == PhotoPickerActivity.MODE_MULTI) {
                ...
                holder.selectView.setTag(photo);
            ...
            /*
            * 读取本地图片，不使用缓存
            * 使用缓存可能会造成的问题是以前的图片文件，现在图片的内容变了，但是文件名字没变
            * 那么就会显示错乱*/
            Glide.with(mContext)
                    .load(photo).into(holder.photoImageView);
//            ImageLoader.getInstance().display(photo, holder.photoImageView,
//                    mWidth, mWidth);
