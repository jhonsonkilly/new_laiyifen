...
public class JpushReceiver extends BroadcastReceiver {
    ...
    public void onReceive(Context context, Intent intent) {
        ...
        if (JPushInterface.ACTION_REGISTRATION_ID.equals(intent.getAction())) {
            ...
            PushFirstRegistEvent msg = new PushFirstRegistEvent();
            msg.id = regId;
...
//            processCustomMessage(context, bundle);
            ...
            Log.d(TAG, "[JpushReceiver] 接收到推送下来的自定义消息: " + str);
            ...
            PushClickEventMessage msg = new PushClickEventMessage();
            msg.intent = intent;
            msg.registId = regId;
            EventBus.getDefault().post(msg);
            //打开自定义的Activity
    ...
    private void processCustomMessage(Context context, Bundle bundle) {
        ...
        try {
            ...
            if (null != extraJson && extraJson.length() > 0) {
                String appUrl = extraJson.getString("url");
                String msgType = extraJson.getString("msgType");
                try {
                    Class c = Class.forName("com.ody.p2p.utils.JumpUtils");
                    Method method = c.getMethod("ToWebActivity", new Class[]{Context.class, String.class});
                    method.invoke(c.newInstance(), context, appUrl);
                } catch (Exception e) {
                    e.printStackTrace();
                }
            }
