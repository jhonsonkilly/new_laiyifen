...
import com.ody.p2p.search.searchresult.SearchResultPresenter;
...
public class StoreAllShopFragment extends BaseFragment implements SearchResultView, View.OnClickListener {
    ...
    public void initPresenter() {
        ...
        mPresenter.getList(key, null, pageNo, shoppingGuideJson, brandIds, priceAnger, navCategoryIds, promotionIds,"");
    }
    ...
    public void initView(View view) {
        showType = TAB_TYPE;
        key = getActivity().getIntent().getStringExtra(Constants.SEARCH_KEY);
        navCategoryNames = getActivity().getIntent().getStringExtra(Constants.NAVCATEGORY_NAME);
        navCategoryIds = getActivity().getIntent().getStringExtra(Constants.NAVCATEGORY_ID);
        promotionIds = getActivity().getIntent().getStringExtra(Constants.PRO_ID);
        if (getActivity().getIntent().getData() != null) {
            String distributeId = getActivity().getIntent().getData().getQueryParameter("distributeId");
            String shareCode = getActivity().getIntent().getData().getQueryParameter("shareCode");
            brandIds = getActivity().getIntent().getData().getQueryParameter("brandId");
        ...
        if (StringUtils.isEmpty(brandIds)) {
            brandIds = getActivity().getIntent().getStringExtra("brandIds");
        }
        brand_icon = getActivity().getIntent().getStringExtra("brandImgUrl");
        brandName = getActivity().getIntent().getStringExtra("brandName");
        if (StringUtils.isEmpty(key) && StringUtils.isEmpty(navCategoryIds) && StringUtils.isEmpty(brandIds)) {
            String keyUrl = getActivity().getIntent().getStringExtra(ActivityRouter.KEY_URL);
