...
public class SlideDetailsLayout extends ViewGroup {
    ...
    public SlideDetailsLayout(Context context, AttributeSet attrs, int defStyleAttr) {
        ...
        mPercent = a.getFloat(R.styleable.SlideDetailsLayout_percen, DEFAULT_PERCENT);
    ...
    public boolean onInterceptTouchEvent(MotionEvent ev) {
        ...
        switch (aciton) {
            case MotionEvent.ACTION_DOWN: {
                ...
                Toast.makeText(getContext(), "1111 ACTION_DOWN", Toast.LENGTH_LONG).show();
                break;
            }
            ...
            case MotionEvent.ACTION_CANCEL: {
                Toast.makeText(getContext(), "1111 ACTION_UP", Toast.LENGTH_LONG).show();
                shouldIntercept = false;
                break;
            }
    ...
    public boolean onTouchEvent(MotionEvent ev) {
        ...
        switch (action) {
            case MotionEvent.ACTION_DOWN: {
                ...
                wantTouch = false;
                Toast.makeText(getContext(), "2222  ACTION_DOWN", Toast.LENGTH_LONG).show();
                break;
            }
            ...
            case MotionEvent.ACTION_CANCEL: {
                finishTouchEvent();
                Toast.makeText(getContext(), "2222  ACTION_UP", Toast.LENGTH_LONG).show();
                wantTouch = false;
                break;
            }
        }
    ...
    protected boolean canChildScrollVertically(int direction) {
        if (mTarget instanceof RecyclerView) {
            ...
            return canListViewSroll((RecyclerView) mTarget);
            ...
            for (int i = 0; i < ((ViewGroup) mTarget).getChildCount(); i++) {
                ...
                if (child instanceof RecyclerView) {
                    return canListViewSroll((RecyclerView) child);
                }
            }
        }
    ...
    protected boolean canListViewSroll(RecyclerView absListView) {
        if (mStatus == Status.OPEN) {
            return absListView.getChildCount() > 0
                    && (((GridLayoutManager) absListView.getLayoutManager()).findFirstVisibleItemPosition() > 0 || absListView.getChildAt(0)
                    .getTop() <
                    absListView.getPaddingTop());
        } else {
            final int count = absListView.getChildCount();
            return count > 0
                    && ((GridLayoutManager) absListView.getLayoutManager()).findLastVisibleItemPosition() < count - 1
