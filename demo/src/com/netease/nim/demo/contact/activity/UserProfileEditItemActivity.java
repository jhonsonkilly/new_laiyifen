...
import com.netease.nim.demo.util.RegexUtils;
...
public class UserProfileEditItemActivity extends UI implements View.OnClickListener {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        if (key == UserConstant.KEY_NICKNAME || key == UserConstant.KEY_PHONE || key == UserConstant.KEY_EMAIL
                || key == UserConstant.KEY_SIGNATURE || key == UserConstant.KEY_ALIAS|| key == UserConstant.KEY_PHONE_REMARK) {
    ...
    private void setTitles() {
        switch (key) {
            ...
            case UserConstant.KEY_ALIAS:
                ...
                break;
            case UserConstant.KEY_PHONE_REMARK:
                setTitle(R.string.phone_number_remark);
                break;
        }
    }
    ...
    private void findEditText() {
        ...
        }else if (key == UserConstant.KEY_PHONE_REMARK) {
            editText.setFilters(new InputFilter[]{new InputFilter.LengthFilter(16)});
        }
        if (key == UserConstant.KEY_ALIAS) {
            ...
            Friend friend = FriendDataCache.getInstance().getFriendByAccount(data);
            if (friend != null && friend.getExtension()!= null&&friend.getExtension().containsKey("phoneNote")) {
                    String remark = friend.getExtension().get("phoneNote")+"";
                    editText.setText(remark);
            } else {
                editText.setHint("请输入备注号码...");
            }
        } else{
    ...
    private void initActionbar() {
        ...
        toolbarView.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                ...
                if (key == UserConstant.KEY_PHONE_REMARK && TextUtils.isEmpty(editText.getText().toString().trim())) {
                    Toast.makeText(UserProfileEditItemActivity.this, R.string.phone_empty, Toast.LENGTH_SHORT).show();
                    return;
                }
                if (key == UserConstant.KEY_PHONE_REMARK && !RegexUtils.isMobilePhoneNumber(editText.getText().toString().trim())) {
                    Toast.makeText(UserProfileEditItemActivity.this, R.string.isphone_number, Toast.LENGTH_SHORT).show();
                    return;
                }
