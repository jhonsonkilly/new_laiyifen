...
public class TeamAUChatAction extends AVChatAction {
//aaa
    ...
    public TeamAUChatAction(AVChatType avChatType) {
        super(avChatType);
    ...
    public void startAudioVideoCall(AVChatType avChatType) {
        ...
        if (AVChatProfile.getInstance().isAVChatting()) {
            Toast.makeText(getActivity(), "正在进行P2P语音通话，请先退出", Toast.LENGTH_SHORT).show();
            return;
        }
            ...
            public void onResult(boolean success, List<TeamMember> result) {
                ...
                if (success && result != null) {
                    if (result.size() < 2) {
                        ...
                        NimUIKit.startContactSelector(getActivity(), getContactSelectOption(tid), TeamRequestCode.REQUEST_TEAM_AUDIO);
                    }
                }
            }
        });
    ...
    private void onCreateRoomSuccess(String roomName, List<String> accounts) {
        ...
        message.setContent(teamNick + getActivity().getString(R.string.t_auchat_start));
        message.setConfig(tipConfig);
        sendMessage(message);
        // 对各个成员发送点对点自定义通知
        String teamName = TeamDataCache.getInstance().getTeamName(transaction.getTeamID());
        String content = TeamAVChatHelper.sharedInstance().buildContent(roomName, teamID, accounts, teamName,"1");
