...
public class OdySwipeRefreshLayout extends ViewGroup {
    ...
    private void action() {
        if (isRefresh) {
            if (odyDefaultFlag) {
                ...
                String time = formatter.format(lastTime);
                text_time_show.setText((lastTime == 0 ? "第一次刷新" : "最后更新:今天" + time));
                if (imageView != null) {
                    initLoading();
...
//                    animationDrawable.start();
    ...
    public OdySwipeRefreshLayout(Context context) {
        this(context, null);
    }

    @SuppressWarnings("deprecation")
    public OdySwipeRefreshLayout(Context context, AttributeSet attrs) {
        super(context, attrs);
        ...
        switch (action) {
            ...
            case MotionEvent.ACTION_MOVE:
                ...
                if (isChildScrollToBottom()) {
                    ...
                    xDiff = Math.abs(mInitialMotionX - x);
                    if (yDiff > mTouchSlop && !mIsBeingDragged && yDiff > xDiff) {// 判断是否下拉的距离足够
            ...
            case MotionEvent.ACTION_MOVE: {
                ...
                if (mIsBeingDragged) {
                    ...
                    if (mOnPushLoadMoreListener != null) {
                        ...
                        if (odyDefaultFlag) {
                            boolean enable = pushDistance >= mFooterViewHeight;
            ...
            case MotionEvent.ACTION_CANCEL: {
                ...
                if (Build.VERSION.SDK_INT < Build.VERSION_CODES.HONEYCOMB) {
                    ...
                    if (odyDefaultFlag) {
                        ...
                        footerTextView.setTextColor(getResources().getColor(R.color.sub_title_color));
    ...
    private boolean odyDefaultFlag = false;
    ...
    private ImageView img_totop_button;

    private long lastTime;


    public void setOdyDefaultView(boolean odyDefaultFlag) {
        this.odyDefaultFlag = odyDefaultFlag;
        if (odyDefaultFlag) {
            setHeaderView(createDefaultHeaderView());
            setFooterView(createDefaultFooterView());
        }
    }

    public void setActionToRefresh(boolean action) {
        this.isRefresh = action;
    }


    private View createDefaultFooterView() {
        View footerView = LayoutInflater.from(getContext()).inflate(R.layout.layout_footer, null);
    ...
    private View createDefaultHeaderView() {
        ...
        if (OdyApplication.SCHEME.equals("lyf")) {
            headerView = LayoutInflater.from(getContext()).inflate(R.layout.lyf_layout_head, null);
            imageView = (ImageView) headerView.findViewById(R.id.iv_loading);
//            initLoading();
        } else {
            headerView = LayoutInflater.from(getContext()).inflate(R.layout.layout_head, null);
            progressBar = (ProgressBar) headerView.findViewById(R.id.pb_view);
            progressBar.setVisibility(View.VISIBLE);
        }
        textView = (TextView) headerView.findViewById(R.id.text_view);
        text_time_show = (TextView) headerView.findViewById(R.id.text_time_show);
        textView.setVisibility(View.VISIBLE);
        text_time_show.setVisibility(View.VISIBLE);
        return headerView;
    }


    public void initLoading() {
        loadAnimationDrawable.animateRawManuallyFromXML(R.drawable.head_loading, imageView, new Runnable() {
            @Override
            public void run() {
