...
//        path = mBitmapTag + path + mBitmapTag;
     ...
     * 插入网络图片
     *
     * @param mFilePath 本地图片路径
     * @param url       网络图片路径
     */
    public void insertUrlBitmap(String mFilePath, String url) {
        Bitmap bitmap = getSmallBitmap(mFilePath, 480, 800);
        insertBitmap(url, bitmap);
//这里走了不少弯路我擦
...
//
//
//        int index = edit_text.getSelectionStart(); // 获取光标所在位置
//        Editable edit_text = edit_text.getEditableText();
//        if (index < 0 || index < edit_text.length()) {
//            edit_text.append(spannableString);
//        } else {
//            edit_text.insert(index, spannableString);
