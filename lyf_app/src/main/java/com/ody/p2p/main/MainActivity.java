...
public class MainActivity extends BaseActivity implements MainView {
    ...
    public void initPresenter() {
        ...
        presenter.getTanPin("newcomers_popup,registered_coupon");
        presenter.initApp(OdySysEnv.getVersionCode(), getPackageName());
        presenter.initCartNum();
        presenter.getUpgrade(OdySysEnv.getVersion(), getPackageName(), OdySysEnv.getVersionCode(), appChannel);
//        presenter.firstLoginCoupon();
        presenter.getCurrDistributor();
...
//       IMRedDotManager.newInstent().init(true);
    ...
    protected void onNewIntent(Intent intent) {
        ...
        //mTODO:meiyizhi DeelLink外链唤醒app（目前放在首页，根据需要，可能要放到其他页面）
        String action = intent.getAction();
        String data = intent.getDataString();
        if (Intent.ACTION_VIEW.equals(action) && data != null) {
            TaklingDataEventMessage msg = new TaklingDataEventMessage();
            msg.setAction(TaklingDataEventMessage.ONRECEIVEDEEPLINK);
    ...
    public void initView(View view) {
        ...
        tab_bar = (TabChooser) view.findViewById(R.id.tab_bar);
        img_select_two = (ImageView) view.findViewById(R.id.img_select_two);
        EventBus.getDefault().register(this);


        view.findViewById(R.id.tv_dev).setOnClickListener(new View.OnClickListener() {
            @Override
                                ...
                                .subscribe(new ApiSubscriber<AreacodeBean>(new SubscriberListener<AreacodeBean>() {
                                    ...
                                    public void onNext(final AreacodeBean o) {
                                        ...
                                        if (!OdyApplication.getString(Constants.PROVINCE, "").equals(o.getData().getTwoAddress())) {
                                            final CustomDialog dialog = new CustomDialog(MainActivity.this, "系统定位您现在在" + location.province + "," + "\n是否要切换到上海", 2);
                ...
                switch (position) {
                    case 0:
                        ...
                        if (Build.MODEL.toUpperCase().contains("OPPO") || Build.MODEL.toUpperCase().contains("LENOVO")) {
                            StatusBarCompat.setStatusBarColor(MainActivity.this, Color.parseColor("#B0B0B0"), true);
                        } else {
                            StatusBarCompat.setStatusBarColor(MainActivity.this, ContextCompat.getColor(MainActivity.this, R.color.white), true);
                        }
...
//
//        NBSAppAgent nbsAppAgent = NBSAppAgent.setLicenseKey("33d8d9f5ec914b739848511acb3563f2").withLocationServiceEnabled(true);
//        nbsAppAgent.enableLogging(false);
//
//        nbsAppAgent.withCrashReportEnabled(tempInt > 1 ? false : true);
...
//        fragments.add(liveFragment);
    ...
    @Override
    public void resume() {
        RetrofitFactory.bundleAlias()
                .subscribeOn(Schedulers.io())
                .subscribeOn(AndroidSchedulers.mainThread())
                .subscribe(new ApiSubscriber<Alias>(new SubscriberListener<Alias>() {
                    @Override
                    public void onError(String msg) {
                        super.onError(msg);
                        JPushInterface.setAlias(MainActivity.this, OdyApplication.getGUID(), new TagAliasCallback() {
                            @Override
                    ...
                    public void onNext(Alias alias) {
                        if (alias != null && alias.code.equals("0")) {
                            if (!StringUtils.isEmpty(alias.data)) {
                                JPushInterface.setAlias(MainActivity.this, alias.data, new TagAliasCallback() {
                                    @Override
                                    public void gotResult(int i, String s, Set<String> set) {
                                    }
                                });
                ...
                rxPermissions.request(
                        ...
                        .subscribe(granted -> {
                            if (granted) {
                                ...
                                Toast.makeText(MainActivity.this, "为了更好的体验，开启存储权限", Toast.LENGTH_SHORT).show();
                            }
                        });
...
//                LogoutHelper.logout();
    ...
    private void refreshUserInfo() {
        ...
        UserUpdateHelper.update(UserInfoFieldEnum.Name, OdyApplication.getValueByKey(Constants.NICK_NAME, OdyApplication.getValueByKey(Constants.LOGIN_MOBILE_PHONE, "")), callback);
        UserUpdateHelper.update(UserInfoFieldEnum.GENDER, "1", callback);
        UserUpdateHelper.update(UserInfoFieldEnum.MOBILE, OdyApplication.getValueByKey(Constants.LOGIN_MOBILE_PHONE, ""), callback);
        UserUpdateHelper.update(UserInfoFieldEnum.AVATAR, OdyApplication.getValueByKey(Constants.HEAD_PIC_URL, ""), callback);
    }
    ...
    private void saveUserInfo(UserInfoBean.Data data) {
        ...
        Log.i(TAG, "user isDistributor? = " + data.isDistributor());
    }
    ...
    public void setPersonalData(PersonalBean response, boolean isImlogin) {
        ...
        if (isImlogin) {
            ...
            LoginHelper.register(MainActivity.this, person.id, person.mobile, person.nickname, OdyApplication.getValueByKey(Constants.USER_LOGIN_UT) + "", person.sex, person.headPicUrl);
            IMRedDotManager.newInstent().init(this);
