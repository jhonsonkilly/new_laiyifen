package com.ody.p2p.settings.utils.pickerview.popwindow;
...
public class DatePickerPopWin extends PopupWindow implements OnClickListener {
    ...
    private int fourTextColor;//外面四层字体颜色
    private int centerTextColor;//中间一层字体颜色
    private int centerLineTextColor;//中间一层上下两条线的颜色
    ...
    public static class Builder{
        ...
        private static int flag;//加flag是为了不同项目局部修改些布局的显示隐藏,0的话什么都不做
        public Builder(int flag,Context context,OnDatePickedListener listener){
        ...
        private int colorTitleBg = Color.parseColor("#303F9F");
        private int btnTextSize = 16;//text btnTextsize of cancel and confirm button
        private int viewTextSize = 25;
        private int fourTextColor = Color.parseColor("#999999");
        private int centerTextColor = Color.parseColor("#999999");
        private int centerLineTextColor = Color.parseColor("#999999");
        ...
        public Builder colorTitleBackground(int colorBg){
            this.colorTitleBg = colorBg;
            return this;
        }

        public Builder setDayVisibility(int visibility){
        ...
        public Builder fourTextColor(int color){
            this.fourTextColor = color;
            return this;
        }
        public Builder centerTextColor(int color){
            this.centerTextColor = color;
            return this;
        }
        public Builder centerLineTextColor(int color){
            this.centerLineTextColor = color;
            return this;
        }
    ...
    public DatePickerPopWin(Builder builder){
        ...
        this.fourTextColor = builder.fourTextColor;
        this.centerTextColor = builder.centerTextColor;
        this.centerLineTextColor = builder.centerLineTextColor;
        this.colorTitleBg = builder.colorTitleBg;
    ...
    private void initView() {
        ...
        contentView = LayoutInflater.from(mContext).inflate(
                R.layout.setting_layout_date_picker, null);
        ...
        container_toolbar = (RelativeLayout) contentView.findViewById(R.id.container_toolbar);
        v_divide_line = contentView.findViewById(R.id.v_divide_line);
        dayLoopView.setVisibility(visibility);
        ...
        container_toolbar.setBackgroundColor(colorTitleBg);
        ...
        yearLoopView.setFourTextColor(fourTextColor);
        monthLoopView.setFourTextColor(fourTextColor);
        dayLoopView.setFourTextColor(fourTextColor);

        yearLoopView.setCenterTextColor(centerTextColor);
        monthLoopView.setCenterTextColor(centerTextColor);
        dayLoopView.setCenterTextColor(centerTextColor);

        yearLoopView.setCenterLineTextColor(centerLineTextColor);
        monthLoopView.setCenterLineTextColor(centerLineTextColor);
        dayLoopView.setCenterLineTextColor(centerLineTextColor);
    ...
    private void initPickerViews() {
        ...
        for (int i = 0; i < yearCount; i++) {
            yearList.add(format2LenStr(minYear + i) + mContext.getString(R.string.year));
        }

        for (int j = 0; j < 12; j++) {
            monthList.add(format2LenStr(j + 1) + mContext.getString(R.string.month));
        }
    ...
    public void setSelectedDate(String dateStr) {
        ...
        if (!TextUtils.isEmpty(dateStr)) {
            ...
            if (milliseconds != -1) {
                ...
                yearPos = calendar.get(Calendar.YEAR) - minYear;//去掉- minYear,年滚动时一直是当前年,不会显示滚动到的年的值
                monthPos = calendar.get(Calendar.MONTH);
                dayPos = calendar.get(Calendar.DAY_OF_MONTH) - 1;//去掉-1也没影响啊....
            }
        }
    }
     ...
     * showAtLocation 里的 Gravity.TOP 这个值为啥都不影响,只会显示在bottom,为何????
     *
     * @param activity
     */
    ...
    public void onClick(View v) {
        ...
        if (v == contentView || v == cancelBtn) {
            ...
            boolean isClosePop = OdyApplication.getValueByKey("isClosePop",false);
            if (isClosePop){
                dismissPopWin();
            }else {
