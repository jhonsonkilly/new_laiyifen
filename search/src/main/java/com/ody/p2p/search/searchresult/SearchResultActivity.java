...
public class SearchResultActivity extends BaseActivity implements SearchResultView, View.OnClickListener {
    ...
    public void initView(View view) {
        ...
        isStore = getIntent().getBooleanExtra("StoreFinish", false);
        navCategoryNames = getIntent().getStringExtra(Constants.NAVCATEGORY_NAME);
        navCategoryIds = getIntent().getStringExtra(Constants.NAVCATEGORY_ID);
        promotionIds = getIntent().getStringExtra(Constants.PRO_ID);
        if (getIntent().getData() != null) {
            ...
            brandIds = getIntent().getData().getQueryParameter("brandId");
        ...
        brand_icon = getIntent().getStringExtra("brandImgUrl");
        ...
        utils = new PopUpWindowUtils(this);
        ...
        swipeRefreshLayout = (OdySwipeRefreshLayout) findViewById(R.id.swipe_refresh);
    ...
    public void onClick(View v) {
        ...
        if (v.equals(iv_showtype)) {
            if (showType == TAB_TYPE) {
                ...
                adapter = new SearchResultAdapter(this, searchResultList, showType);
                if (loadCom) {
        ...
        if (v.equals(iv_cartCar)) {
            ...
            final com.ody.p2p.views.basepopupwindow.SelectMenu menu = new com.ody.p2p.views.basepopupwindow.SelectMenu(SearchResultActivity.this, popList);
        ...
        if (bean != null && bean.data != null) {
            ...
            if (bean.data.productList != null && bean.data.productList.size() > 0) {
                if (pageNo == 1) {
                    ...
                    if (showType == TAB_TYPE) {
                        recycler_seachreuslt.setLayoutManager(gridLayoutManager);
                        adapter = new SearchResultAdapter(this, searchResultList, TAB_TYPE);
                        recycler_seachreuslt.setAdapter(adapter);
                        adapter.setAddBuyCart(new SearchResultAdapter.AddBuyCart() {
                            @Override
                            public void addtoCart(String mpId, int position) {
...
//        ToastUtils.showToast(getString(R.string.add_succeed));
        Toast mToast = Toast.makeText(this, getString(R.string.add_succeed), Toast.LENGTH_SHORT);
